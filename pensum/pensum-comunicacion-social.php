...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0,10,'ID DE CARRERA: 7 - COMUNICACION SOCIAL',0,1,'C');
        $this->Ln(5);
    }
...
$sql = "SELECT semestre, cod_asignatura, nombre_asignatura, horas_teoricas, horas_practicas, horas_semanales, UC, prelaciones  FROM pensum_comunicacion_social ORDER BY semestre, cod_asignatura";
    ...
    while ($row = $result->fetch_assoc()) {
        if ($previous_semester != $row['semestre']) {
            if ($previous_semester != '') {
                $pdf->Ln(38); // Agregar una fila para separar los semestres
            }
