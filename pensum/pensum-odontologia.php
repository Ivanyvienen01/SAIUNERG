...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0,10,'ID DE CARRERA: 2 - ODONTOLOGIA',0,1,'C');
        $this->Ln(5);
    }
...
$sql = "SELECT año, cod_asignatura, nombre_asignatura, horas_teoricas, horas_practicas, horas_semanales, UC, prelaciones  FROM pensum_medicina ORDER BY año, cod_asignatura";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        if ($previous_semester != $row['año']) {
            if ($previous_semester != '') {
                $pdf->Ln(20); // Agregar una fila para separar los años
            }
            // Agregar la fila con el número de año
            $pdf->Cell(195, 10, utf8_decode('Año ') . $row['año'], 1, 1, 'C');
            $pdf->Row(array('Cod.', 'Nombre de la Asignatura', 'Horas Teoricas', 'Horas Practicas', 'Horas Semanales', 'U.C', 'prelaciones'));
            $previous_semester = $row['año'];
