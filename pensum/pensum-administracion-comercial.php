...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0,5,'AREA DE CIENCIAS ECONOMICAS Y SOCIALES',0,1,'C');
        $this->Cell(0,10,'PENSUM DE ESTUDIO ',0,1,'C');
        $this->Cell(0,5,'ID DE CARRERA: 5 - ADMINISTRACION COMERCIAL',0,1,'C');
        $this->Ln(0);
    }
...
$sql = "SELECT semestre, cod_asignatura, nombre_asignatura, horas_teoricas, horas_practicas, horas_semanales, UC, prelaciones  FROM pensum_administracion_comercial ORDER BY semestre, cod_asignatura";
$result = $conn->query($sql);

// Verificar si se obtuvieron resultados
if ($result->num_rows > 0) {
    // Definir anchos de columna
    $pdf->SetWidths(array(25, 45, 20, 20, 20, 20, 45));
    ...
    while ($row = $result->fetch_assoc()) {
        if ($previous_semester != $row['semestre']) {
            if ($previous_semester != '') {
                $pdf->Ln(7); // Agregar una fila para separar los semestres
            }
