...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Ln(10);
        $this->Cell(0, 10, 'REPORTE DE SOLICITUDES DE PREINSCRIPCION', 0, 1, 'C');
        $this->Ln(5);
    }
...
$pdf->AliasNbPages();
$pdf->SetMargins(5, 10, 20); // Márgenes
...
// Consulta SQL para obtener datos de la tabla estudiantes
$sql = "SELECT * FROM estudiantes";
$result = $conn->query($sql);

// Verificar si se obtuvieron resultados
if ($result->num_rows > 0) {
    // Definir anchos de columna (incluyendo la nueva columna para el número ascendente)
    $pdf->SetWidths(array(8, 22, 22, 20, 30, 20, 20, 22, 22, 24, 20, 20, 20, 20));

    // Definir alineaciones (incluyendo la nueva columna)
    ...
    // Agregar el encabezado de la tabla con la nueva columna
    $pdf->HeaderRow(array('#', 'NOMBRES', 'APELLIDOS', 'CEDULA', 'CORREO', 'CARRERA', 'INGRESO', 'FECHA INSCRIP.', 'FECHA NAC.', 'TELF.', 'ESTADO / USUARIO', 'GENERO', 'ESTADO CIVIL', 'ESTADO'));

    // Reiniciar el número ascendente antes de agregar filas
    $pdf->ReiniciarNumeroAscendente();
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $data = array($numero_ascendente, $row["nombres"], $row["apellidos"], $row["cedula"], $row["correo"], $row["nombre_carrera"], $row["tipo_ingreso"], $row["fecha_inscripcion"], $row["fecha_nacimiento"], $row["telefono"], $row["condicion"], $row["genero"], $row["estado_civil"], $row["estado"]);
        $pdf->Row($data);
    }
} else {
