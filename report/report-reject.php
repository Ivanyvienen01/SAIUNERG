...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Ln(10);
        $this->Cell(0, 10, 'REPORTE DE SOLICITUDES DE PREINSCRIPCION RECHAZADAS', 0, 1, 'C');
        $this->Ln(5);
    }
...
$pdf->AliasNbPages();
$pdf->SetMargins( 10, 10, 20); // Márgenes
...
// Consulta SQL para obtener datos de la tabla preinscripciones
$sql = "SELECT * FROM preinscripciones_rechazadas";
$result = $conn->query($sql);

// Verificar si se obtuvieron resultados
if ($result->num_rows > 0) {
    // Definir anchos de columna (incluyendo la nueva columna para el número ascendente)
    $pdf->SetWidths(array(10, 30, 40, 20, 45, 30, 30, 25, 25, 25));
    ...
    $pdf->HeaderRow(array('#', 'NOMBRES', 'APELLIDOS', 'CEDULA', 'CORREO', 'CARRERA', 'INGRESO', 'FECHA INSCRIP.', 'FECHA RECHAZO.', 'ESTADO / USUARIO'));
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $data = array($numero_ascendente, $row["nombres"], $row["apellidos"], $row["cedula"], $row["correo"], $row["nombre_carrera"], $row["tipo_ingreso"], $row["fecha_inscripcion"], $row["fecha_rechazo"], 'Inactivo');
        $pdf->Row($data);
    }
} else {
